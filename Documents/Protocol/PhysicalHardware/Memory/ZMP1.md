...
17. 提供 §4.23 的线性位元端口，§4.24 的小端整数端口，§4.25 的块内八位组与块内整数，以及 §4.26 的 `Owner` / `Claim` / `Release`；
...
- `MemoryInit` 上除 `Test` 与 `Initialize` 以外没有别的必选方法。内存总控与内存块上已经写死的方法见 §4.19–§4.26，**禁止**把那些方法宣称为本条尚未锁定。
...
`IntegerValue` 是非负整数，范围 `0 .. 2^(8×OctetWidth) − 1`。
